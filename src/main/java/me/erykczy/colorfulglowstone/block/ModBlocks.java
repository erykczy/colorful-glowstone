...
public class ModBlocks {
    ...
    public static final HashMap<DyeColor, DeferredBlock<Block>> GLOWSTONES = new HashMap<>();
    public static final HashMap<DyeColor, DeferredItem<BlockItem>> GLOWSTONE_BLOCK_ITEMS = new HashMap<>();

    static {
        for(var dyeColor : DyeColor.values()) {
            GLOWSTONES.put(dyeColor, registerGlowstone(dyeColor));
    ...
    private static DeferredItem<BlockItem> registerGlowstoneItem(DyeColor color) {
        return BLOCK_ITEMS.registerSimpleBlockItem(GLOWSTONES.get(color));
    }
